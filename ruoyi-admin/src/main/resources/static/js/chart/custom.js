    ...
    $.ajax({
        ...
        "success": function (data) {
            ...
            echartBar1.setOption({
                ...
                // series[0].data=yaxis1,
                // series[1].data=yaxis2,
                series : [
                    {
                        type: 'bar',
                        data:yaxis1,
        ...
        "success": function (data) {
            if(data.code==0){
                for(var i=0;i<data.rows.length;i++){
                    let ob = JSON.parse(data.rows[i].content);
                    ob.id=data.rows[i].calendarId;
                    events.push(ob);
                }
            }
        }
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        select: function(arg) {
            ...
            $(".antosubmit").on("click", function() {
                ...
                if (title) {
                    let id = calenderAddPost(title,arg);
                    calendar.addEvent({
                        id:id,
                        title: title,
                        start: started,
        ...
        eventClick: function(calEvent ) {
            $(".antodelete").off('click');
            $(".antosubmit2").off('click');
            $('#fc_edit').click();
            $('#title2').val(calEvent.event.title);
            $(".antodelete").on("click", function() {
                $('#deleteCalender').modal('show');
                $(".antodeleteConfirm").on("click", function() {
                    calEvent.event.remove();
                    calenderDeletePost(calEvent);
                    $('.antoclose2').click();
                    $('#deleteCalender').modal('hide');
                });
            });
            $(".antosubmit2").on("click", function() {
                calEvent.event.setProp('title', $("#title2").val());
                calenderEditPost(calEvent);
                $('.antoclose2').click();
            });
            calendar.unselect();
        },
...
function calenderEditPost(info){
    var ob = {};
    ob.allDay=info.event.allDay;
    ob.title=info.event.title;
    ob.start=info.event.start;
    ob.end=info.event.end;
    JSON.stringify(ob);
    var data={};
    data.calendarId=info.event.id;
    data.content= JSON.stringify(ob);
    data.startTime=JSON.stringify(info.event.start);
    if(info.event.end!=null){
        data.endTime=JSON.stringify(info.event.end);
    }
    $.ajax({
        "type": "POST",
        "data":data,
        "url": "/system/calendar/edit",
        "async": true,
        "success": function (data) {
...
function calenderDeletePost(info){
    $.ajax({
        "type": "POST",
        "data":{id:info.event.id},
        "url": "/system/calendar/remove",
        "async": true,
        "success": function (data) {
            console.log(data);
        }
...
function calenderAddPost(title,info){
    var result;
    $.ajax({
        "type": "POST",
        "dataType":"json",
        "data":{content:JSON.stringify({title:title,start:info.startStr}),
            startTime:info.startStr,
            endTime:info.endStr},
        "url": "/system/calendar/add",
        "async": false,
        "success": function (data) {
            result = data.msg;
        }
